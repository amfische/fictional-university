...
function university_files() {
  ...
  wp_enqueue_style('theme-meta-data', get_stylesheet_uri());
...
function university_custom_rest() {
  ...
  register_rest_field('professor', 'page_banner_subtitle', [
    'get_callback' => function () {
...
add_filter('wp_insert_post_data', function ($data) {
  ...
  if ($data['post_type'] === 'note' && count_user_posts(get_current_user_id(), 'note') > 4 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $errorMsg = 'You have reached your note limit.';
    wp_is_json_request() ? wp_send_json_error(['msg' => $errorMsg], 400) : die($errorMsg);
  }
